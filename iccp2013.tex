...
\begin{equation}
 \Pi_c(\Theta,k)=\frac{a}{|{\bf V}(k)|}
     \iiint_{{\bf V}_c(\Theta,k)}
     \frac{1}{[R({\bf v})]^2} d{\bf v}
  \;\;,
  \label{eq:Pc}
\end{equation}
where $a$ is the effective area of the camera lens and $|{\bf V}(k)|$ is the volume 
of voxel $k$. The distance $R({\bf v})$ is measured between the camera and an infinitesimal volume element at ${\bf v}\in {\bf V}_c(\Theta,k)$.
...
We simulated two different spatial distributions. Both were set as
...
The second function, $w_2$, expresses a clustered nature of aerosol ``clouds''. In the first simulation we define two blobs in the form of ellipsoids: the first is
centered at altitude
$3.3km$, and has horizontal width of $32km$ and vertical thickness of $2.8km$. The second ellipsoid is centered at altitude $6.6km$, and has lateral width of $24km$ and vertical thickness of $2.1km$.
In the second simulation we define front in the form of a lying down half cylinder. The half cylinder is centered at altitude $5km$, and has lateral width of $32km$ and vertical thickness of $4km$.
Then
\begin{equation}
  w_2(k) =
  \left\{
      \begin{array}{ll}
          1  & \mbox{ if $k\in$ any ``clouds'' cluster} \\
          0  & \mbox{ otherwise}
    \label{eq:f2}
       \end{array}
  \right.
...
Fig.~\ref{fig:simulation-results1} shows two modeled images from the simulation of the haze-blobs. In total the simulation uses 95 modeled images.
\begin{figure}
  \centering
  \yoavcomment{\includegraphics[width=0.8\columnwidth]{images/ref_images.pdf}}
  \caption{\small Rendered sky photographs (contrast enhanced) corresponding
   to different viewpoints. A yellow dot marks
    the Sun location. Dashed white circles mark elevation angels.}
  \label{fig:simulation-results1}
\end{figure}

Fig.~\ref{fig:simulation1} shows an iso-surface visualization of the original
haze-blobs distribution and its reconstruction. Fig.~\ref{fig:simulation2}
shows an iso-surface visualization of the original haze-front distribution and
its reconstruction. Fig.~\ref{fig:objective} shows the progression of the
objective as a function of the iteration number. As can be seen by the
graph, convergence occurs after several thousand iterations.

To estimate the error of the reconstruction we calculate per voxel the
difference between real and estimated number of particles. We sum the
absolute values of the differences and then normalize by the total number
of particles in the real distribution. The reconstruction error for the
haze-blobs simulation is 0.202 and for the haze-front simulation is 0.198.
\begin{figure}
  \centering
  \yoavcomment{\includegraphics[width=\columnwidth]{images/simulation1}}
  \caption{\small [a] Simulated 3D aerosol distribution composed of haze-blobs. [b] Its reconstruction. Color represents aerosol density. The density units are
...
\begin{figure}
  ...
  \caption{\small [a] Simulated 3D aerosol distribution composed of a haze-front. [b] Its reconstruction. Color represents aerosol density. The density units are
  $10^{15}~{\rm particles}/{\rm cm}^3.$}
